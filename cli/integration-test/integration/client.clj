...
(defrecord Client [client-id
  ...
  (receive-response [this {:keys [id] :as resp}]
    (if-let [{:keys [request start-ns]} (get @sent-requests id)]
      (let [ms (/ (double (- (. System (nanoTime)) start-ns)) 1000000.0)]
        (do (protocols.endpoint/log this :green (format "received response (ms %s):" ms) resp)
...
(defn request-and-await-server-response! [client method body]
  (let [timeout-ms 120000
        resp (time (deref (protocols.endpoint/send-request client method body)
                          timeout-ms
                          ::timeout))]
    (if (= ::timeout resp)
      (do
